...
class MeshGraphNetWithContext(MeshGraphNet):
    def __init__(self, input_dim_nodes, input_dim_edges, output_dim, context_dim, processor_size=15):
        super(MeshGraphNetWithContext, self).__init__(input_dim_nodes, input_dim_edges, output_dim, processor_size)
        self.context_dim = context_dim
        self.node_encoder = MeshGraphMLP(input_dim_nodes + context_dim, hidden_dim=128, output_dim=128)
...
def main(cfg: DictConfig) -> None:
    ...
    model = MeshGraphNetWithContext(  # Updated class
        ...
        input_dim_edges=4,
        output_dim=4,
        context_dim=2,  # Add context dimension
        ...
        for i in range(len(graphs)):
            ...
            for j in range(cfg.num_partitions):
                with torch.autocast(amp_device, enabled=True, dtype=amp_dtype):
                    ...
                    context_file_path = os.path.join(cfg.partitions_path, f"global_context_{i}.npy")
            ...
            for i in range(len(validation_graphs)):
                ...
                for j in range(cfg.num_partitions):
                    ...
                    with torch.no_grad():
                        with torch.autocast(amp_device, enabled=True, dtype=amp_dtype):
                            # Load global context for the current partition
                            context_file_path = os.path.join(cfg.validation_partitions_path, f"global_context_{i}.npy")
