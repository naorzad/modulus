...
def compute_cl_cd(vtp_file: str):
    ...
    coordinates = point_cloud.points[:, :2] 
    ...
    # Compute pitching moment coefficient around quarter-chord point (0.25, 0)
    ref_point = np.array([0.25, 0.0])
    dx = coordinates[:, 0] - ref_point[0]
    dy = coordinates[:, 1] - ref_point[1]
    Cmy_pred = np.sum(dx * C_total_pred[:, 1] - dy * C_total_pred[:, 0])
    Cmy_true = np.sum(dx * C_total_true[:, 1] - dy * C_total_true[:, 0])
