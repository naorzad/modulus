                ...
                with torch.autocast(amp_device, enabled=True, dtype=amp_dtype):
                    ...
                    ndata = torch.cat(
                        (
                            ...
                            part.ndata["Mach"].view(-1, 1),  # Ensure 1D tensLors are concatenated properly
                    ...
                    target = torch.cat((part.ndata["pressure"], part.ndata["shear_stress"]), dim=1)  # Shape: (num_nodes, 4)
        ...
        if dist.rank == 0 and epoch % cfg.validation_freq == 0:
            ...
            for i in range(len(validation_graphs)):
                ...
                shear_stress_pred = torch.zeros(
                    (sum_unique_nodes, 3), dtype=torch.float32, device=device  # Change to 2D
                )
                ...
                point_cloud.field_data["Mach"] = Mach_denorm[0].numpy()
                point_cloud.field_data["AOA"] = AOA_denorm[0].numpy()
                point_cloud.field_data["ReL"] = ReL_denorm[0].numpy()
