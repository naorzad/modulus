...
def split_data(run_dirs, train_ratio=0.9, val_ratio=0.07, test_ratio=0.03):
...
def process_run(
    ...
    try:
        ...
        graph.ndata["coordinates"] = torch.tensor(np.column_stack((all_points, np.zeros(all_points.shape[0]))), dtype=torch.float32) # Add zero Z-coordinate
        graph.ndata["normals"] = torch.tensor(np.column_stack((all_normals, np.zeros(all_points.shape[0]))), dtype=torch.float32)
