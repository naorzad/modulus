...
def plot_test_cases(vtp_files, output_dir):
    ...
    for vtp_file in vtp_files:
        ...
        coordinates = point_cloud.points
...
def evaluate_test_data(cfg: DictConfig, model_checkpoint: str, output_dir: str = "test_point_clouds"):
    ...
    for i, subgraphs in enumerate(test_graphs):
        ...
        shear_stress_pred = torch.zeros((sum_unique_nodes, 3), dtype=torch.float32, device=device)
        pressure_true = torch.zeros((sum_unique_nodes, 1), dtype=torch.float32, device=device)
        shear_stress_true = torch.zeros((sum_unique_nodes, 3), dtype=torch.float32, device=device)
        coordinates = torch.zeros((sum_unique_nodes, 3), dtype=torch.float32, device=device)
        normals = torch.zeros((sum_unique_nodes, 3), dtype=torch.float32, device=device)
        ...
        for j in range(cfg.num_partitions):
            ...
            edata = torch.cat(
            (
                part.edata["x"],      # (num_edges, edge_feature_dim)
                torch.full((part.number_of_edges(), 1), part.ndata["Mach"][0].item()).to(device),
                torch.full((part.number_of_edges(), 1), part.ndata["ReL"][0].item()).to(device),
                torch.full((part.number_of_edges(), 1), part.ndata["AOA"][0].item()).to(device),
            ),
            dim=1,
            )
            with torch.no_grad():
                with autocast(amp_device, enabled=True, dtype=amp_dtype):
                    pred = model(ndata, edata, part)
        ...
        coordinates_denorm = coordinates_denorm.numpy()
        normals_denorm = normals_denorm.numpy()
        shear_stress_pred_denorm = shear_stress_pred_denorm.numpy()
        shear_stress_true_denorm = shear_stress_true_denorm.numpy()
...
def main(cfg: DictConfig) -> None:
    ...
    if 1: plot_cl_cd_comparison(cl_cd_results_list,output_dir)
    if 1: plot_cl_cd_colorVars(cl_cd_results_list,output_dir)
    plot_coefficients_vs_aoa(cl_cd_results_list,output_dir)
    if 1: plot_test_cases(vtp_files, output_dir)
