...
from dataloader_globalFeat import create_dataloader
from utils import (
    find_bin_files,
    save_checkpoint,
    load_checkpoint,
        ...
        if dist.rank == 0 and epoch % cfg.validation_freq == 0:
            ...
            for i in range(len(validation_graphs)):
                ...
                for j in range(cfg.num_partitions):
                    ...
                    Mach, ReL, AOA = part.global_context_unnorm
                    ndata = torch.cat(
                        (
                            part.ndata["coordinates"],
                            part.ndata["normals"],
                    ...
                    (
                        part.edata["x"],      # (num_edges, edge_feature_dim)
                        part.edata["globalFeat"], # (num_edges, 3)
                    ),
                    dim=1,
                    )
                ...
                coordinates_denorm = (coordinates_denorm.numpy(), np.zeros((coordinates_denorm.shape[0], 1)))
                normals_denorm = (normals_denorm.numpy(), np.zeros((normals_denorm.shape[0], 1)))
                shear_stress_pred_denorm = (shear_stress_pred_denorm.numpy(), np.zeros((shear_stress_pred_denorm.shape[0], 1)))
                shear_stress_true_denorm = (shear_stress_true_denorm.numpy(), np.zeros((shear_stress_true_denorm.shape[0], 1)))
                ...
                point_cloud.field_data["Mach"] = np.array([Mach])
                point_cloud.field_data["AOA"] = np.array([AOA])
                point_cloud.field_data["ReL"] = np.array([ReL])
