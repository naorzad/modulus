...
def process_partition(graph, num_partitions, halo_hops):
    ...
    for _, subgraph in partitioned.items():
        ...
        subgraph.__dict__["global_context"] = graph.__dict__["global_context"]
        subgraph.__dict__["global_context_unnorm"] = graph.__dict__["global_context_unnorm"]
...
def process_run(
    ...
    try:
        ...
        all_normals = calculate_3d_normals(all_points)  # Calculate normals
    ...
    try:
        ...
        graph.__dict__["global_context"] = torch.tensor(global_context, dtype=torch.float32)
        graph.__dict__["global_context_unnorm"] = torch.tensor(global_context_unnorm, dtype=torch.float32)
        ...
        dgl.save_graphs(partition_file_path, partitioned_graphs)
        print("Before saving:")
        for key, value in graph.__dict__.items():
            print(f"{key}")
        loaded_graphs, _ = dgl.load_graphs(partition_file_path)
        print("\nAfter loading:")
        ...
        if save_point_cloud:
            ...
            # Store global context in field data
