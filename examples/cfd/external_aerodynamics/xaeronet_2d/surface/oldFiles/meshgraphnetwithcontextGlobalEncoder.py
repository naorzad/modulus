...
class MeshGraphEdgeMLPConcatWithContext(nn.Module):
    ...
    def forward(
        ...
        with graph.local_scope():
            graph.ndata['n'] = nfeat
            graph.ndata['f'] = flow_features  # Per-node flow features
            graph.edata['e'] = efeat
            graph.apply_edges(
                lambda edges: {
                    'cat': torch.cat(
...
class MeshNodeBlockWithContext(nn.Module):
    def __init__(
        ...
        # Input to MLP includes aggregated edge features, node features, and per-node flow features
    ...
    def forward(
        ...
        cat_feat_with_context = torch.cat([cat_feat, flow_features], dim=-1)
        nfeat_new = self.node_mlp(cat_feat_with_context) + nfeat  # Residual connection
...
class MeshGraphNetWithContext(nn.Module):
    ...
    def forward(
        ...
        # Ensure context_features match the number of nodes
        assert context_features.size(0) == graph.number_of_nodes(), \
            "Context features must have shape (num_nodes, input_dim_context)"

        # Encode features
        node_features = self.node_encoder(node_features)
        edge_features = self.edge_encoder(edge_features)
        flow_features = self.context_encoder(context_features)  # Per-node context features
...
# cfeat = torch.randn(3, 8)  # Per-node context features
...
# print(out.shape)  # Should print torch.Size([3, 2])