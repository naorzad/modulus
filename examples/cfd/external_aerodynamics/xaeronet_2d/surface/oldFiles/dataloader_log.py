...
class GraphDataset(Dataset):
    ...
    def __init__(self, file_list, mean, std, normalize):
    ...
    def __getitem__(self, idx):
        ...
        # Apply log transformation to pressure and shear_stress fields
            if self.normalize.get("pressure", True):
                graph.ndata["pressure"] = torch.log(graph.ndata["pressure"] + 1)  # Adding 1 to avoid log(0)
            if self.normalize.get("shear_stress", True):
                graph.ndata["shear_stress"] = torch.sign(graph.ndata["shear_stress"]) * torch.log(torch.abs(graph.ndata["shear_stress"]) + 1)
            ...
            # self.normalize edge data
            if self.normalize.get("x", True):
                if "x" in graph.edata:
                    graph.edata["x"] = (
            ...
            # print(len(graph.ndata["pressure"]))
...
def create_dataloader(
    ...
    """
    dataset = GraphDataset(file_list, mean, std, normalize)
