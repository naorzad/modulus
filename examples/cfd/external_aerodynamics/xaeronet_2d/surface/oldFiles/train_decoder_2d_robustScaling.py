            ...
            for i in range(len(validation_graphs)):
                ...
                if normalize.get("pressure", True):
                    pressure_pred_denorm = (
                        pressure_pred.cpu() * torch.tensor(iqr_vals["pressure"])
                    ) + torch.tensor(median_vals["pressure"])
                    pressure_true_denorm = (
                        pressure_true.cpu() * torch.tensor(iqr_vals["pressure"])
                    ) + torch.tensor(median_vals["pressure"])
                else:
                ...
                if normalize.get("shear_stress", True):
                    shear_stress_pred_denorm = (
                        shear_stress_pred.cpu() * torch.tensor(iqr_vals["shear_stress"])
                    ) + torch.tensor(median_vals["shear_stress"])
                    shear_stress_true_denorm = (
                        shear_stress_true.cpu() * torch.tensor(iqr_vals["shear_stress"])
                    ) + torch.tensor(median_vals["shear_stress"])
                else:
                ...
                if normalize.get("coordinates", True):
                    coordinates_denorm = (
                        coordinates.cpu() * torch.tensor(iqr_vals["coordinates"])
                    ) + torch.tensor(median_vals["coordinates"])
                else:
                    coordinates_denorm = coordinates.cpu()

                if normalize.get("normals", True):
                    normals_denorm = (
                        normals.cpu() * torch.tensor(iqr_vals["normals"])
                    ) + torch.tensor(median_vals["normals"])
                else:
                    normals_denorm = normals.cpu()

                if normalize.get("area", True):
                    area_denorm = (
                        area.cpu() * torch.tensor(iqr_vals["area"])
                    ) + torch.tensor(median_vals["area"])
                else:
