...
class GraphDataset(Dataset):
    ...
    def __init__(self, file_list, mean, std):
        ...
        self.mach_mean = torch.tensor(mean["Mach"])
        self.mach_std = torch.tensor(std["Mach"])
        self.rel_mean = torch.tensor(mean["ReL"])
        self.rel_std = torch.tensor(std["ReL"])
        self.aoa_mean = torch.tensor(mean["AOA"])
        self.aoa_std = torch.tensor(std["AOA"])
    ...
    def __getitem__(self, idx):
        ...
        for graph in graphs:
            ...
            graph.ndata["Mach"] = (graph.ndata["Mach"] - self.mach_mean) / self.mach_std
            graph.ndata["ReL"] = (graph.ndata["ReL"] - self.rel_mean) / self.rel_std
            graph.ndata["AOA"] = (graph.ndata["AOA"] - self.aoa_mean) / self.aoa_std
